...
namespace Backdrop\oauth2_server;

use php\src\OAuth2\OpenID\Storage\AuthorizationCodeInterface;
use php\src\OAuth2\OpenID\Storage\UserClaimsInterface;
use php\src\OAuth2\Storage\AccessTokenInterface;
use php\src\OAuth2\Storage\ClientCredentialsInterface;
use php\src\OAuth2\Storage\JwtBearerInterface;
use php\src\OAuth2\Storage\PublicKeyInterface;
use php\src\OAuth2\Storage\RefreshTokenInterface;
use php\src\OAuth2\Storage\UserCredentialsInterface;

/**
 * Provides Backdrop storage (through the underlying Entity API) for the library.
 */
  ...
  PublicKeyInterface
{
  ...
  public function getClientDetails($client_key) {
    ...
    if ($client) {
      ...
      $client = array(
        ...
        // Backdrop behavior in other areas.
  ...
  public function getAuthorizationCode($code) {
    ...
    if ($code) {
      ...
      // https://www.backdrop.org/node/2274357#comment-9779467
      $sub_property = config_get('oauth2_server.settings','oauth2_server_user_sub_property')??'uid';
  ...
  public function checkUserCredentials($username, $password) {
    ...
    if ($account && $account->status) {
      require_once BACKDROP_ROOT . '/' . config_get('oauth2_server.settings','password_inc')??'core/includes/password.inc';
  ...
  public function getUserClaims($uid, $scope) {
    ...
    // See: https://www.backdrop.org/node/2274357#comment-9779467
    $sub_property = config_get('oauth2_server.settings','oauth2_server_user_sub_property')??'uid';
    ...
    if (in_array('email', $requested_scopes)) {
      ...
      $claims['email_verified'] = config_get('oauth2_server.settings','user_email_verification')??true;
    }
    ...
    if (in_array('profile', $requested_scopes)) {
      ...
      if (user_access('access user profiles', backdrop_anonymous_user())) {
    ...
    backdrop_alter('oauth2_server_user_claims', $claims, $account, $requested_scopes);
  ...
  public function getRefreshToken($refresh_token) {
    $token = oauth2_server_token_load($refresh_token, 'refresh');
    if ($token) {
      $token_wrapper = entity_metadata_wrapper('oauth2_server_token', $token);
      $scopes = array();
      foreach ($token_wrapper->scopes as $scope_wrapper) {
  ...
  protected function getUserPicture($account) {
    if (!config_get('oauth2_server.settings','user_pictures')??0) {
      return NULL;
    }
    ...
    if (!empty($account->picture)) {
      ...
      $picture_path = config_get('oauth2_server.settings','user_picture_default')??null;
    }
    ...
    if (module_exists('image') && file_valid_uri($picture_path) && $style = config_get('oauth2_server.settings','user_picture_style')??'') {
